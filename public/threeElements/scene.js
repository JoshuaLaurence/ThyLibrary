...
manager.onStart = function (url, itemsLoaded, itemsTotal) {
	if (startingLoad) {
		...
		loadingTip.innerText =
			wittyLoadingTips[Math.floor(Math.random() * wittyLoadingTips.length)];
		console.log(
...
function init() {
	...
	controls.addEventListener("unlock", function () {
		if (
			...
			window.getComputedStyle(informationScreen, null).display === "none" &&
				...
				if (specificIntersection.object.userData.type === "Book") {
					if (moveToCamera[0] === true) {
						if (moveToCamera.length === 2) {
							...
							const pos = specificIntersection.object.getWorldPosition(
								new THREE.Vector3()
							);
							const rot = specificIntersection.object.getWorldQuaternion(
